...
% --- Per-komponent strøm i HVER af de 3 sleep-scenarier ---------------
% Disse værdier bruges i Panel 2 (grouped horizontal bar chart).  Kun
% ESP'en ændrer sig mellem scenarierne; alle andre komponenter er
% konstante (de har ikke en analog "sleep mode").
%
% Sleep-mode ESP-currents:
%   modem-sleep: CPU on, RF kun aktiv ~2s pr 15-min cyklus -> ~15 mA gns.
%   light-sleep: CPU pauset mellem events                  -> ~1 mA gns.
T_event_cycle_pre = T_check_poll;   % 15 min mellem events
t_event_act_pre   = 2.0;            % sek aktiv pr. event
duty_event_pre    = t_event_act_pre / T_event_cycle_pre;

I_esp_modem_pre = duty_event_pre*I_esp_3v3 + (1-duty_event_pre)*15e-3;
I_esp_light_pre = duty_event_pre*I_esp_3v3 + (1-duty_event_pre)*0.9e-3;

% Matrix: rows = komponenter, cols = [no_sleep, modem, light]   (mA)
comp_per_scen_mA = [
    I_esp_3v3,     I_esp_modem_pre, I_esp_light_pre;       % ESP — den eneste der ændrer sig
    comp.OLED,     comp.OLED,       comp.OLED;             % OLED — uændret
    comp.LM35,     comp.LM35,       comp.LM35;             % LM35
    comp.HC_SR04,  comp.HC_SR04,    comp.HC_SR04;          % HC-SR04
    comp.Stepper,  comp.Stepper,    comp.Stepper;          % Stepper
    comp.ULN_quiet,comp.ULN_quiet,  comp.ULN_quiet         % ULN2003
] * 1e3;
...
% --- Panel 2: horisontal bar chart ------------------------------------
% ESP-rækken får 3 sub-søjler (no sleep / modem / light) fordi det er
% den eneste komponent der ændrer sig med sleep-mode.
% De andre komponenter er konstante og vises som ÉN søjle hver.
ax2 = nexttile;

% Find ESP-row og separer
esp_idx     = find(strcmp(comp_names_disp,'ESP8266 (WiFi)'),1);
other_idx   = setdiff(1:numel(comp_names_disp), esp_idx);
esp_vals    = comp_per_scen_mA(esp_idx,:);       % [no_sleep, modem, light]
other_vals  = comp_per_scen_mA(other_idx,1);     % konstant — tag bare no_sleep kolonne
other_names = comp_names_disp(other_idx);

% Sortér de øvrige descending efter forbrug
[other_vals_s, sidx_o] = sort(other_vals, 'descend');
other_names_s = other_names(sidx_o);

n_others = numel(other_vals_s);
n_total  = 1 + n_others;     % ESP-row + andre

% --- ESP sub-søjler omkring y=1 -------------------------------------
groupwidth = 0.85;
nESP       = 3;
sub_h      = groupwidth / nESP;
y_off_esp  = 1 - groupwidth/2 + (2*(1:nESP)-1)*groupwidth/(2*nESP);

esp_colors = [0.85 0.33 0.10;   % no sleep — orange (current state)
              0.95 0.75 0.20;   % modem    — gul
              0.30 0.65 0.40];  % light    — grøn (best)

hold(ax2,'on');
hL = gobjects(nESP,1);          % handles til legend
for i = 1:nESP
    hL(i) = barh(ax2, y_off_esp(i), esp_vals(i), sub_h, ...
                 'FaceColor', esp_colors(i,:), 'EdgeColor','none');
end

% --- Andre komponenter: én søjle hver ved y = 2, 3, ... -------------
other_color = [0.45 0.55 0.65];     % neutral grå-blå (konstant = ikke optimerbar)
for k = 1:n_others
    barh(ax2, k+1, other_vals_s(k), groupwidth, ...
         'FaceColor', other_color, 'EdgeColor','none');
end
hold(ax2,'off');

% --- Akse-setup ------------------------------------------------------
y_labels = [{'ESP8266 (WiFi)'}, other_names_s];
set(ax2, 'YDir','reverse', ...
         'YTick', 1:n_total, ...
         'YTickLabel', y_labels, ...
         'FontSize',12,'LineWidth',1.0,'Box','off');
xlabel(ax2,'Current draw on the 5V rail (mA)','FontSize',13);
ax2.XGrid='on'; ax2.YGrid='off'; ax2.GridAlpha = 0.25;
ax2.XScale = 'linear';
ax2.XLim   = [0 85];
ax2.XTick  = 0:10:80;
ax2.YLim   = [0.5 n_total+0.5];

% --- Legend (forklarer kun ESP-farverne — andre er én farve = "konstant") ---
legend(ax2, hL, {'ESP — no sleep','ESP — modem sleep','ESP — light sleep'}, ...
       'Location','southeast','FontSize',11,'Box','off');

% --- Værdi-labels ved hver søjle ------------------------------------
% ESP — 3 labels (én pr. sub-søjle)
for i = 1:nESP
    v = esp_vals(i);
    if v >= 1, lbl = sprintf(' %.1f', v); else, lbl = sprintf(' %.2f', v); end
    text(ax2, v, y_off_esp(i), lbl, ...
         'HorizontalAlignment','left','VerticalAlignment','middle', ...
         'FontSize',9,'Color',[0.15 0.15 0.15]);
end
% Andre — 1 label pr. komponent
for k = 1:n_others
    v = other_vals_s(k);
    if v >= 1, lbl = sprintf(' %.1f', v); else, lbl = sprintf(' %.2f', v); end
    text(ax2, v, k+1, lbl, ...
...
ylabel(ax3,'Batterylifetime (log-scale)','FontSize',13);
ax3.YScale = 'log';
...
ax3.YTick = [1 6 12 24 72 168 720];
ax3.YTickLabel = {'1 t','6 h','12 h','1 day','3 days','1 week','1 month'};
