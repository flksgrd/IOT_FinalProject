...
The brain of the bin is an \textbf{ESP8266 NodeMCU} module - it has Wifi-capabilities, which allows us to run the intended program. It was not, however, the ideal candidate: the ESP8266 only exposes a handful of usable GPIO pins and several of those have boot-time roles, which made the pin layout one of the recurring headaches of the project. However we manged to keep the entire system limited to a single ESP-unit, making the system cheaper to produce and sell.
\\ \\
...
\end{align*}
\noindent where $I_{s}$ and $t_{s}$ are the average current and the time spent in state $s$ over one day $T$, $V_{5\text{V}}$ is the regulated rail voltage, $\eta$ the buck-converter efficiency, $I_{q}$ the buck and BMS quiescent current, and $Q_{\text{batt}}$ and $V_{\text{batt}}$ the battery charge and nominal voltage.
