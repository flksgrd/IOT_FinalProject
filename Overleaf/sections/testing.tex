...
\\ \\
At the pipeline level the full chain was run continuously for multi-hour sessions across several lab days. ESP pushes appeared on the dashboard within 5--16\,s, a new bag count entered on the dashboard reached the ESP within the predicted 50\,s worst case, bag-full and low-bags events arrived as phone notifications within 30\,s, and the ONLINE/OFFLINE pill flipped to OFFLINE within 90\,s of unplugging the ESP.
\\ \\
The system has known limitations that we did not address. HTTP traffic to ThingSpeak is unencrypted (acceptable for non-sensitive telemetry but not for production); the WiFi credentials are hard-coded in the firmware (a real product would use a setup portal to set wifi credentials); the 15\,s minimum between ThingSpeak writes is fine for one bin in a kitchen but would be a problem for a commercial multi-bin deployment; and the bin-full threshold is fixed at 10\,cm rather than calibrated to the bin's geometry. Natural next steps would be to swap ThingSpeak for a self-hosted MQTT broker with TLS, replace the LM35 with a combined humidity/gas sensor to warn about spoiling organic waste, with a small mobile app instead of leaning on IFTTT, and add a fleet view for households with multiple bins.
